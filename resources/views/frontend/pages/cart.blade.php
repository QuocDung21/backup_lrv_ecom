                                    ...
                                    @foreach ($cart as $item)
                                        <tr class="tr-{{$item['cart_id']}}">
                                            <td class="product-thumbnail"><a
                                                    href="/shop/product/{{$item['cart_id']}}-{{Str::slug($item['cart_product'], '-')}}.html"><img
                                                        style="width: 100px; height: 120px;object-fit: cover"
                                                        src="{{$item['cart_image']}}" alt="product img"/></a></td>
                                            <td class="product-name"><a
                                                    href="/shop/product/{{$item['cart_id']}}-{{Str::slug($item['cart_product'], '-')}}.html">{{$item['cart_product']}}</a>
                                            </td>
                                            <td class="product-price"><span
                                                    class="amount">{{number_format($item['cart_price_sale'])}}</span>
                                            </td>
                                            <form>
                                                @csrf
                                                <td class="product-quantity"><input class="get_qty"
                                            ...
                                            <td class="product-subtotal"
                                                name='total_{{$item['cart_id']}}'>{{number_format($item['cart_price_sale'] * $item['cart_quantity'])}}</td>
                                            <td class="product-remove">
                                                <form>
                                                    @csrf
                                                    <button type="button" class="cart_delete"
                                                            data-id_delete="{{$item['cart_id']}}">
